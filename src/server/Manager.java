...
public class Manager extends Thread{
	...
	public Manager(PacoteSincronizado pac, DatagramSocket socketServidor) {
		this.pac = pac;
	...
	public void run(){
		...
		while (!isInterrupted()) {
			...
			if(usuario.getAcao().equals("Logar")){//Caso o usuario deseje logar
				try {
					arq = new FileReader("usuarios" + File.separator + usuario.getPlayerName().trim() + ".ser");
					...
					try{
					    FileOutputStream fo = new FileOutputStream("usuarios" + File.separator + usuario.getPlayerName() + ".ser");
					    ObjectOutputStream oo = new ObjectOutputStream(fo);
					    oo.writeObject(usuario);//Cria um arquivo com o nome do usuario
					    oo.close();
					}catch(IOException ex){
						System.err.println("Erro na serialização do arquivo." + usuario.getPlayerName());
					}
				}
				...
				if(usuario.getAcao().equals("Desafiar")){//Caso o usuario busque o ip do adversario
					...
					for (Player obj : listJogadores) {  //Busca na lista de jogadores o ip e porta do adversario
					    if(usuario.getPlayerName().equals(obj.getPlayerName())){
					    	ipAdversario = obj.getiP();
					    	porta = obj.getPorta();
					    	adversario = obj.getPlayerName();
